  ...
  PostgreSQL.prototype.addIndexes = function(model, actualIndexes, cb) {
    ...
    aiNames.forEach(function(indexName) {
      ...
      if (
        ...
        if (propName) {
          ...
          const siKeys = si.keys.map((item) => item[0]);
    ...
    propNames.forEach(function(propName) {
      ...
      if (!found) {
        ...
        let statement =
          'CREATE ' + kind + ' INDEX ' + iName + ' ON ' + self.tableEscaped(model) + type + ' ( ' + pName + ')';

        if (i.options && i.options.where) {
          statement += ' WHERE ' + i.options.where;
        }
